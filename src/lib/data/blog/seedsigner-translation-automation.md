...
date: 2026-08-17
summary: Keeping source strings, translations and review screenshots in sync across three repositories, and the trust model that makes it safe to run on untrusted pull requests.
---

[SeedSigner](https://github.com/SeedSigner/seedsigner) is an open-source, air-gapped Bitcoin signing device that runs on cheap, off-the-shelf parts. Its interface is translated by volunteers, and for my second Summer of Bitcoin I automated the localization workflow around that: the machinery that keeps the app's translatable text, its translations, and the screenshots translators review in sync across three separate repositories.
